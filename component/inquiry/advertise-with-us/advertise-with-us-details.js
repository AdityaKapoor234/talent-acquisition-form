...
export default class AdvertiseWithUsDetails extends Component {
	constructor(props) {
		super(props);
		this.state = {
			tab: 1,
			advertiseWithUs: props?.advertiseWithUs,
			mode: props?.mode,
		};
	}
	static getDerivedStateFromProps(nextProps, prevState) {
		if (
			prevState.advertiseWithUs !== nextProps.advertiseWithUs ||
			prevState.mode !== nextProps.mode
		) {
			return {
				advertiseWithUs: nextProps?.advertiseWithUs,
				mode: nextProps?.mode,
			};
		}
		return null;
									...
									<div className="row mt-4">
										<div className="col-md-4">
											<div className="login-form ">
												...
												<input
													type="text"
													value={this.state.advertiseWithUs?.name}
													readOnly={true}
												/>
											</div>
											<div className="login-form ">
												...
												<input
													type="text"
													value={this.state.advertiseWithUs?.email}
													readOnly={true}
												/>
											</div>
											<div className="login-form ">
											...
											<div className="login-form ">
												...
												<textarea
													cols="100"
													rows="5"
													value={this.state.advertiseWithUs?.short_description}
													readOnly={true}
												/>
											</div>
										</div>
										{/* <div className="col-md-12 ">
											...
											<div className="signup-check">
												<div className="d-flex flex-wrap login-form"> */}
													{/* {this.state.advertiseWithUs?.expert?.expertises?.map((value) => {
														return (
															<FormGroup>
																<FormControlLabel
																	control={
													...
													{/* <textarea
														cols="100"
														rows="5"
														value={this.state.advertiseWithUs?.expert?.expertises?.map((value) => value?.name)?.join(" , ")}
														readOnly={true}
													/>
												</div>
											</div>
