...
export default function SportsViewDetails({ id }) {
  const mode = "view";

  const [sports, setSports] = useState([]);
  const [open, setOpen] = useState(false);

  const sportsDetail = (id) => {
    SportsApi.getSportsViewDetails(id)
      .then((response) => {
        setSports(response.data.data.sport);
      })
      .catch((error) => {
        toast.error(
          error?.response &&
        ...
        <DashboardLayoutComponent>
          <div className="row border-box">
            <div className="col-md-5">
              <div className="hamburger">
                <span>Catalog / Sports/ </span>View Sports
              </div>
              <div className="page-name">Sports Details - {sports?.name}</div>
            </div>
          ...
          <div className="row">
            <div className="col-m-12">
              <SportsCreateComponent sports={sports} mode={mode} />
            </div>
          </div>
        </DashboardLayoutComponent>
        <Dialog
