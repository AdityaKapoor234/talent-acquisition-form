...
export default class CancelDetails extends Component {
	constructor(props) {
		super(props);
		this.state = {
			tab: 1,
	...
	static getDerivedStateFromProps(nextProps, prevState) {
		if (
			prevState.orders !== nextProps.orders ||
			...
			return {
				...
				input: {
					name: nextProps?.order?.name,
					email: nextProps?.order?.email,
					avatar_url: nextProps?.order?.avatar_url,
					is_active: nextProps?.order?.is_active,
					experience: nextProps?.order?.experience,
									...
									<div className="row mt-4">
										<div className="col-md-4">
											<div className="login-form ">
												...
												<input
													type="text"
													value={this.convertDateStringToDate(this.state.orders?.submitted_at)}
													readOnly={true}
												/>
											</div>
											<div className="login-form ">
												...
												<input
													type="text"
													value={this.state.orders?.subject}
													readOnly={true}
												/>
											</div>
											<div className="login-form ">
												...
												<input
													type="text"
													value={this.state.orders?.expert?.name}
													readOnly={true}
												/>
											</div>
											<div className="mt-4">
												<Photo
													...
													label={"Avatar"}
													accept=".jpg,.jpeg,.png"
													img={this.state.orders?.expert?.avatar_url}
												/>
											</div>
											<div className="login-form ">
												<label>Description<span className="mandatory-star">*</span></label>
											...
											<div className="login-form ">
												...
												<textarea
													cols="100"
													rows="5"
													value={this.state.orders?.expert?.expertises?.map((value) => value?.name)?.join(" , ")}
													readOnly={true}
												/>
											</div>
										</div>
										{/* <div className="col-md-12 ">
											...
											<div className="signup-check">
												<div className="d-flex flex-wrap login-form"> */}
													{/* {this.state.orders?.expert?.expertises?.map((value) => {
														return (
															<FormGroup>
																<FormControlLabel
																	control={
